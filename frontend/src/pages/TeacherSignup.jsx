...
export default function TeacherSignup() {
  ...
  const handleSignupSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMessage(err.message || 'CNIC verification failed. Please ensure your CNIC is registered with the admin settings.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slateCustom-950 flex flex-col justify-between relative font-sans overflow-hidden transition-colors duration-300">
      ...
      <main className="flex-grow flex items-center justify-center p-6 z-10">
        <motion.div 
          ...
          className="w-full max-w-[480px] bg-white dark:bg-slateCustom-900 border border-slate-200 dark:border-slateCustom-800 rounded-[36px] shadow-2xl p-8 sm:p-10 relative overflow-hidden"
        >
          ...
          <div className="text-center space-y-2 mb-6">
            ...
            <h1 className="font-outfit font-black text-2xl tracking-tight text-slate-900 dark:text-white block">Teacher Registration</h1>
            <p className="text-slate-400 text-xs font-semibold">Verify CNIC with school records to create account</p>
          </div>
          ...
          <form onSubmit={handleSignupSubmit} className="space-y-4 text-left">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full h-11 pl-11 pr-4 rounded-xl border border-slate-200 dark:border-slateCustom-800 bg-slate-50 dark:bg-slateCustom-950 text-slate-800 dark:text-white text-sm outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 transition-all"
                />
              </div>
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">CNIC Identity Card Number *</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
              ...
              <p className="text-[9px] text-slate-400 italic">Note: Must exactly match the CNIC entered by the school Admin.</p>
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">Email Address (For login) *</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">System Password *</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                ...
                <input 
                  ...
                  placeholder="••••••••••••"
                  className="w-full h-11 pl-11 pr-12 rounded-xl border border-slate-200 dark:border-slateCustom-800 bg-slate-50 dark:bg-slateCustom-950 text-slate-800 dark:text-white text-sm outline-none focus:ring-1 focus:ring-teal-500 focus:border-teal-500 transition-all"
                />
                <button 
                  type="button"
            ...
            <AnimatePresence>
              {errorMessage && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -5 }}
                  className="p-3 bg-rose-50 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/60 rounded-xl text-rose-600 dark:text-rose-400 text-xs font-semibold"
                >
                  {errorMessage}
                </motion.div>
              )}
              ...
              {successMessage && (
                <motion.div 
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -5 }}
                  className="p-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-900/60 rounded-xl text-emerald-600 dark:text-emerald-400 text-xs font-semibold"
                >
                  {successMessage}
                </motion.div>
              )}
      ...
      <footer className="h-16 flex items-center justify-center text-xs text-slate-400 z-10 px-6 text-center">
        <span>© {new Date().getFullYear()} Pakhtunkhwa Model School. Secure CNIC Registry Verification.</span>
      </footer>
    </div>
  );
}
