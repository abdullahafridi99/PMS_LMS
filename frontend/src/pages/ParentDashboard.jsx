...
export default function ParentDashboard() {
  ...
  const [selectedChild, setSelectedChild] = useState(null); // Active child object
  ...
  const loadParentDashboardDetails = async (parent) => {
    ...
    try {
      // 1. Fetch children profiles based on childrenEmails
      const childrenEmails = parent.childrenEmails || [];
      const allStudents = await api.students.list();
      
      const matchedChildren = allStudents.filter(student => 
        childrenEmails.includes(student.email) || student.parentEmail === parent.email
      );

      setChildren(matchedChildren);

      // 2. Fetch notices targeted to parents/everyone
      const fetchedNotices = await api.notices.list();
      setNotices(fetchedNotices);

      // 3. Load first child if exists
      if (matchedChildren.length > 0) {
        setSelectedChild(matchedChildren[0]);
        await loadChildData(matchedChildren[0]);
      }
  ...
  const loadChildData = async (child) => {
    ...
    try {
      // Fetch Child Attendance Statistics
      const att = await api.attendance.getStudentStats(childId);
      setAttendanceStats(att.summary);
      setAttendanceRecords(att.records);

      // Fetch Child Invoices
      const fetchedInvoices = await api.fees.list(childId);
  ...
  // Payment trigger simulator with premium Canvas Confetti explosion
  ...
  const handleProcessPaymentSubmit = async (e) => {
    ...
    try {
      ...
      // Wait 1.5 seconds to simulate secure bank validation
      setTimeout(() => {
        setPayLoading(false);
        setShowPayModal(false);
        
        // Trigger celebratory premium confetti explosion!
        confetti({
          particleCount: 150,
          spread: 80,
          origin: { y: 0.6 }
  ...
  // Mock Grades report card for children
  const childrenGrades = [
    { subject: 'Mathematics', total: 100, scored: 86, grade: 'A' },
    { subject: 'English Grammar', total: 100, scored: 91, grade: 'A+' },
    { subject: 'Physics', total: 100, scored: 88, grade: 'A' },
    { subject: 'Chemistry', total: 100, scored: 74, grade: 'B' },
    { subject: 'Computer Science', total: 100, scored: 95, grade: 'A+' },
    { subject: 'Islamic Studies', total: 100, scored: 90, grade: 'A+' },
    { subject: 'Pakistan Studies', total: 100, scored: 82, grade: 'A' },
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slateCustom-950 font-sans flex transition-colors duration-300">
      ...
      <aside className="w-64 bg-slate-900 text-slate-400 p-6 flex flex-col justify-between shrink-0 border-r border-slate-800">
        <div className="space-y-8 text-left">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <div>
              ...
              <span className="text-[9px] font-bold text-emerald-400 block tracking-widest uppercase">Parent Panel</span>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <button 
              ...
              className={`w-full h-11 px-4 rounded-xl font-semibold text-sm flex items-center gap-3 transition-colors ${
                activeTab === 'overview' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button 
              ...
              className={`w-full h-11 px-4 rounded-xl font-semibold text-sm flex items-center gap-3 transition-colors ${
                activeTab === 'attendance' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button 
              ...
              className={`w-full h-11 px-4 rounded-xl font-semibold text-sm flex items-center gap-3 transition-colors ${
                activeTab === 'grades' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button 
              ...
              className={`w-full h-11 px-4 rounded-xl font-semibold text-sm flex items-center gap-3 transition-colors ${
                activeTab === 'fees' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
            ...
            <button 
              ...
              className={`w-full h-11 px-4 rounded-xl font-semibold text-sm flex items-center gap-3 transition-colors ${
                activeTab === 'notices' ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/20' : 'hover:bg-slate-800 hover:text-white'
              }`}
            >
        ...
        <div className="space-y-4 text-left">
          {parentUser && (
            <div className="px-3">
              <span className="text-xs font-bold text-white block truncate">{parentUser.name}</span>
              <span className="text-[9px] text-emerald-400 block font-semibold uppercase tracking-wider">Guardian Account</span>
              <span className="text-[9px] text-slate-500 block truncate">{parentUser.email}</span>
            </div>
          )}
          <button 
      ...
      <main className="flex-grow p-8 overflow-y-auto max-h-screen">
        ...
        {/* HEADER */}
        <header className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center mb-8 text-left">
          <div>
            <h1 className="font-outfit font-black text-3xl tracking-tight text-slate-900 dark:text-white uppercase">
              {activeTab === 'overview' && 'Children Summary Hub'}
              {activeTab === 'attendance' && 'Child Attendance Logs'}
              {activeTab === 'grades' && 'Academic Performance Sheets'}
              {activeTab === 'fees' && 'School Fees & Invoices'}
              {activeTab === 'notices' && 'Official Notice Board'}
            </h1>
            <p className="text-sm text-slate-400 mt-1">
              Guardian Control Room • Zangali Peshawar
            </p>
          </div>

          {/* Child Selector Dropdown */}
          {children.length > 0 && (
            <div className="flex items-center gap-3 bg-white dark:bg-slateCustom-900 border border-slate-200 p-2 rounded-2xl shadow-sm">
              <span className="text-xs font-bold text-slate-400 pl-2">Child Profile:</span>
                ...
                {children.map(child => (
                  <button 
                    ...
                    className={`px-4 h-9 rounded-xl text-xs font-bold transition-all ${
                      selectedChild && (selectedChild.id === child.id || selectedChild._id === child._id)
                        ? 'bg-emerald-500 text-white shadow-sm'
                        : 'bg-slate-50 dark:bg-slateCustom-950 text-slate-600 dark:text-slate-400 hover:bg-slate-100'
                    }`}
                  >
                    {child.name.split(' ')[0]}
                  </button>
        ...
        {globalLoading ? (
          <div className="w-full h-[60vh] flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm font-semibold text-slate-400">Syncing guardian profiles...</p>
          </div>
        ) : (
          <div className="space-y-8">
            
            {/* CHILD DETAILED VIEWS */}
            {selectedChild ? (
              childLoading ? (
                ...
                <div className="space-y-8">
                  ...
                  {activeTab === 'overview' && (
                    <div className="grid lg:grid-cols-12 gap-8 text-left">
                      {/* Left: Quick Child Profile Card */}
                      <div className="lg:col-span-4 bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 shadow-sm space-y-6">
                        <div className="text-center space-y-3">
                          <div className="w-20 h-20 bg-emerald-100 dark:bg-emerald-950/60 rounded-full flex items-center justify-center mx-auto border-2 border-emerald-500">
                            <span className="font-outfit font-black text-2xl text-emerald-600 dark:text-emerald-400">
                              {selectedChild.name.slice(0, 2).toUpperCase()}
                            </span>
                          </div>
                          <div>
                            <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white">{selectedChild.name}</h3>
                            <span className="text-xs font-semibold text-emerald-500 uppercase tracking-widest">{selectedChild.rollNumber}</span>
                          </div>
                        </div>

                        <div className="border-t border-slate-100 dark:border-slateCustom-800 pt-4 space-y-3 text-xs">
                          <div className="flex justify-between">
                            <span className="text-slate-400">Registered Class</span>
                            <strong className="text-slate-700 dark:text-slate-300">{selectedChild.class} - {selectedChild.section}</strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-400">Enrollment Date</span>
                            <strong className="text-slate-700 dark:text-slate-300">
                              {selectedChild.admissionDate ? new Date(selectedChild.admissionDate).toLocaleDateString() : 'N/A'}
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-slate-400">Student System Email</span>
                            <strong className="text-slate-700 dark:text-slate-300">{selectedChild.email}</strong>
                          </div>
                        </div>
                      </div>

                      {/* Right: Quick Performance Summary */}
                      <div className="lg:col-span-8 space-y-6">
                        {/* Summary metrics row */}
                        <div className="grid sm:grid-cols-3 gap-6">
                          {/* Item 1 */}
                          <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                              <span className="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider">Attendance %</span>
                              <strong className="text-xl font-black text-slate-800 dark:text-white font-outfit">{attendanceStats.attendancePercentage}%</strong>
                            </div>
                          </div>
                          {/* Item 2 */}
                          <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                              <span className="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider">Midterm Average</span>
                              <strong className="text-xl font-black text-slate-800 dark:text-white font-outfit">86.2%</strong>
                            </div>
                          </div>
                          {/* Item 3 */}
                          <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 shadow-sm flex items-center gap-4">
                            ...
                            <div>
                              <span className="text-[10px] font-semibold text-slate-400 block uppercase tracking-wider">Unpaid Fee Invoices</span>
                              <strong className="text-xl font-black text-rose-500 font-outfit">
                                {invoices.filter(i => i.status === 'unpaid').length} Bills
                              </strong>
                            </div>
                          </div>
                        </div>

                        {/* Recent Notices Board Feed */}
                        <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 shadow-sm">
                          <h4 className="font-outfit font-bold text-base text-slate-800 dark:text-white flex items-center gap-2 mb-4">
                            <Megaphone className="w-5 h-5 text-emerald-500" /> Official Parent Advisory Notice
                          </h4>
                          
                          {notices.length > 0 ? (
                            <div className="space-y-3">
                              <span className="text-[9px] font-bold bg-emerald-100 text-emerald-600 dark:bg-emerald-950 px-2 py-0.5 rounded-full uppercase tracking-wider block w-fit">
                                {notices[0].targetAudience} Notice
                              </span>
                              <h5 className="font-bold text-sm text-slate-800 dark:text-white">{notices[0].title}</h5>
                              <p className="text-xs text-slate-400 leading-relaxed">{notices[0].content}</p>
                              <button onClick={() => setActiveTab('notices')} className="text-xs font-semibold text-emerald-500 hover:underline flex items-center gap-1 mt-2">
                                View notice board feed <ChevronRight className="w-4 h-4" />
                              </button>
                            </div>
                          ) : (
                            <p className="text-xs text-slate-400 font-medium">No parent notifications posted.</p>
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  {activeTab === 'attendance' && (
                    <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 text-left shadow-sm space-y-6">
                      <div className="flex justify-between items-center border-b border-slate-100 dark:border-slateCustom-800 pb-4">
                        <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                          <Clock className="w-5.5 h-5.5 text-emerald-500" /> Attendance Ledger for {selectedChild.name}
                        </h3>
                        <span className="text-xs font-bold bg-emerald-50 dark:bg-emerald-950 px-3.5 py-1.5 rounded-xl text-emerald-600 border border-emerald-100 dark:border-emerald-900/60">
                          Total Percentage: {attendanceStats.attendancePercentage}%
                        </span>
                      </div>
                  ...
                  {activeTab === 'grades' && (
                    <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 text-left shadow-sm space-y-6">
                      <div className="flex justify-between items-center border-b border-slate-100 dark:border-slateCustom-800 pb-4">
                        <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                          <Award className="w-5.5 h-5.5 text-emerald-500" /> Academic Report Card for {selectedChild.name}
                        </h3>
                      </div>
                        ...
                        <table className="w-full text-sm text-left">
                          <thead className="text-xs uppercase bg-slate-50 dark:bg-slateCustom-950 font-bold tracking-wider text-slate-500 border-b border-slate-100 dark:border-slateCustom-800">
                            <tr>
                              ...
                              <th className="px-6 py-4 text-center">Scored Marks</th>
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-slateCustom-800 text-slate-700 dark:text-slate-300">
                            {childrenGrades.map((card, idx) => (
                              <tr key={idx} className="hover:bg-slate-50/50 dark:hover:bg-slateCustom-950/20 transition-colors">
                                ...
                                <td className="px-6 py-4 text-center font-semibold font-outfit">{card.total}</td>
                                <td className="px-6 py-4 text-center font-semibold font-outfit">{card.scored}</td>
                                <td className="px-6 py-4 text-center font-black text-slate-900 dark:text-white font-outfit">{card.scored}%</td>
                                <td className="px-6 py-4 text-center">
                                  <span className={`px-2.5 py-1 rounded-lg text-xs font-black font-outfit shadow-sm bg-emerald-100 text-emerald-600 dark:bg-emerald-950/60 dark:text-emerald-400`}>
                                    {card.grade}
                                  </span>
                                </td>
                              </tr>
                  ...
                  {activeTab === 'fees' && (
                    <div className="bg-white dark:bg-slateCustom-900 border border-slate-200/50 dark:border-slateCustom-800 rounded-3xl p-6 text-left shadow-sm space-y-6">
                      <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slateCustom-800 pb-4">
                        <Receipt className="w-5.5 h-5.5 text-emerald-500" />
                        <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white">Monthly Fee Bills and Invoices</h3>
                      </div>
                        ...
                        <table className="w-full text-sm text-left">
                          <thead className="text-xs uppercase bg-slate-50 dark:bg-slateCustom-950 font-bold tracking-wider text-slate-500 border-b border-slate-100 dark:border-slateCustom-800">
                            <tr>
                              ...
                              <th className="px-6 py-4 text-center">Status</th>
                              <th className="px-6 py-4 text-right">Payment Portal Action</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 dark:divide-slateCustom-800/60 text-slate-700 dark:text-slate-300">
                            {invoices.length > 0 ? (
                                  ...
                                  <td className="px-6 py-4 text-right">
                                    {invoice.status === 'unpaid' ? (
                                      <button 
                                        ...
                                        className="px-4 py-1.5 bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs rounded-xl shadow-md shadow-emerald-500/10 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-1.5 ml-auto pt-2"
                                      >
                              ...
                              <tr>
                                <td colSpan="7" className="text-center py-12 text-slate-400 font-medium">No recorded fee invoices found for this child profile.</td>
                              </tr>
                            )}
                          </tbody>
                        </table>
              ...
              <div className="w-full h-80 bg-white dark:bg-slateCustom-900 border border-slate-200 dark:border-slateCustom-800 rounded-3xl flex items-center justify-center p-6 text-center text-slate-400 font-medium">No linked child profile detected.</div>
            )}
            ...
            {activeTab === 'notices' && (
              <div className="space-y-6 text-left max-w-4xl">
                <div className="flex items-center gap-2 mb-2">
                  <Megaphone className="w-5.5 h-5.5 text-emerald-500" />
                  <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white">Guardian Bulletins Notice board</h3>
                </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
            ...
            >
              <div className="h-16 px-8 bg-slate-50 dark:bg-slateCustom-950 flex items-center justify-between border-b border-slate-100 dark:border-slateCustom-800">
                <h3 className="font-outfit font-bold text-lg text-slate-800 dark:text-white flex items-center gap-2">
                  <CreditCard className="w-5 h-5 text-emerald-500" /> PMS Secure Payment Gateway
                </h3>
              ...
              <form onSubmit={handleProcessPaymentSubmit} className="p-8 space-y-6">
                    ...
                    <div className={`w-full h-44 rounded-3xl p-6 flex flex-col justify-between shadow-lg relative overflow-hidden text-white ${
                      ...
                      <div className="flex justify-between items-center text-xs">
                        <div>
                          ...
                          <strong className="tracking-wide">{parentUser ? parentUser.name : 'Guardian Profile'}</strong>
                        </div>
                ...
                <div className="bg-slate-50 dark:bg-slateCustom-950 border border-slate-200/50 dark:border-slateCustom-800 p-4 rounded-2xl flex justify-between items-center text-xs">
                  ...
                  <div className="text-right">
                    ...
                    <strong className="text-emerald-500 font-bold font-outfit text-sm">PKR {selectedInvoice ? selectedInvoice.amount : '0.00'}</strong>
                  </div>
                </div>
                    ...
                    <>
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slateCustom-800 bg-slate-50 dark:bg-slateCustom-950 text-slate-800 dark:text-white text-sm outline-none focus:border-emerald-500"
                        />
                      </div>
                      <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-left">
                        <div className="space-y-2 col-span-2">
                          ...
                          <input 
                            ...
                            placeholder="e.g. Muhammad Jamil Khan"
                            className="w-full h-11 px-4 rounded-xl border border-slate-200 dark:border-slateCustom-800 bg-slate-50 dark:bg-slateCustom-950 text-slate-800 dark:text-white text-sm outline-none focus:border-emerald-500"
                          />
                        </div>
                        <div className="space-y-2 col-span-2">
                ...
                <div className="flex justify-end gap-3 pt-2">
                  ...
                  <button 
                    type="submit" 
                    disabled={payLoading}
                    className="flex-grow h-12 bg-emerald-500 hover:bg-emerald-600 text-white font-semibold rounded-xl text-sm shadow-md flex items-center justify-center gap-2 pt-1"
                  >
                    {payLoading ? (
                      <>
      ...
      <AnimatePresence>
        {showChallan && selectedInvoice && selectedChild && (
          <div className="fixed inset-0 z-[100] bg-white dark:bg-slateCustom-950 overflow-y-auto p-8 flex flex-col items-center">
            ...
            <div className="max-w-[1100px] w-full flex justify-between items-center mb-6 pb-4 border-b dark:border-slateCustom-800 print:hidden">
              <h3 className="font-outfit font-black text-xl text-slate-855 dark:text-white uppercase tracking-tight flex items-center gap-2">
                ...
                <div key={copyIdx} className="flex-1 px-5 space-y-4 text-xs font-sans text-left">
                  ...
                  <div className="grid grid-cols-2 gap-y-1.5 text-[10px] border-b pb-3">
                    ...
                    <div className="col-span-2 mt-1">
                      ...
                      <strong className="text-slate-855 font-black">{selectedChild?.studentId} / {selectedChild?.rollNumber}</strong>
                    </div>
                    <div className="col-span-2">
                      <span className="text-slate-500 block">Student Full Name:</span>
                      <strong className="text-slate-855 font-black uppercase">{selectedChild?.name}</strong>
                    </div>
                    <div>
                      <span className="text-slate-500 block">Class - Section:</span>
                      <strong className="text-slate-855 font-black">{selectedChild?.class} - {selectedChild?.section}</strong>
                    </div>
                    <div>
