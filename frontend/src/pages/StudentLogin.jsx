...
export default function StudentLogin() {
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setErrorMessage(err.message || 'Incorrect Student ID or Password');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slateCustom-950 flex flex-col justify-between relative font-sans overflow-hidden transition-colors duration-300">
      ...
      <main className="flex-grow flex items-center justify-center p-6 z-10">
        ...
        >
          <div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-cyan-400 to-cyan-600" />
          ...
          <div className="text-center space-y-2 mb-8">
            ...
            <h1 className="font-outfit font-black text-2xl tracking-tight text-slate-900 dark:text-white block">Student Portal</h1>
            <p className="text-slate-400 text-xs font-semibold">Pakhtunkhwa Model School Zangali Branch</p>
          </div>

          <form onSubmit={handleLoginSubmit} className="space-y-5">
            <div className="space-y-2 text-left">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400 block">Unique Student ID *</label>
            ...
            <div className="space-y-2 text-left">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  placeholder="••••••••••••"
                  className="w-full h-12 pl-11 pr-12 rounded-xl border border-slate-200 dark:border-slateCustom-800 bg-slate-50 dark:bg-slateCustom-950 text-slate-800 dark:text-white text-sm outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500 transition-all"
                />
                <button 
                  type="button"
