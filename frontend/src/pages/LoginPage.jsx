...
import { ShieldCheck, UserCheck, HeartHandshake, ArrowLeft, ChevronRight, GraduationCap } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const portals = [
    ...
    },
    {
      ...
      description: 'Educational console for teachers to check scan logs, check-in, and manage school announcements.',
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slateCustom-950 flex flex-col justify-between relative font-sans overflow-hidden transition-colors duration-300">
      ...
      {/* PORTALS LIST */}
      <main className="flex-grow flex flex-col items-center justify-center p-6 z-10 max-w-7xl mx-auto w-full">
          ...
          {portals.map((portal, idx) => {
            ...
            return (
              <motion.div
                ...
                className="bg-white dark:bg-slateCustom-900 border border-slate-200 dark:border-slateCustom-800 rounded-[32px] shadow-xl p-6 flex flex-col justify-between relative overflow-hidden group hover:shadow-2xl transition-all"
              >
                <div className={`absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r ${portal.color}`} />
                
                <div className="space-y-4 text-left mb-6">
                  <div className={`w-12 h-12 rounded-2xl ${portal.bgColor} flex items-center justify-center shadow-sm`}>
                    <Icon className={`w-6 h-6 ${portal.iconColor}`} />
                  </div>
                  <div>
                    <h3 className="font-outfit font-bold text-xl text-slate-900 dark:text-white mb-2 leading-tight">{portal.title}</h3>
                    <p className="text-slate-500 dark:text-slate-400 text-[11px] leading-relaxed font-medium">{portal.description}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3 w-full">
                  ...
                  {portal.title === 'Teacher Portal' && (
                    <button
                      ...
                      className="w-full h-11 border border-slate-200 dark:border-slateCustom-800 hover:bg-slate-50 dark:hover:bg-slateCustom-950 text-slate-750 dark:text-slate-300 font-bold text-xs rounded-xl flex items-center justify-center transition-all uppercase tracking-wider"
                    >
